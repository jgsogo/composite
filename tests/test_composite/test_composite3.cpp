...
namespace {
    ...
    using Trait1 = composite::Trait<Data1, Data1>;
    using Trait2 = composite::Trait<Data2, Data2>;
    using Trait3 = composite::Trait<Data3, Data3>;

    inline void onPartAdded(const std::vector<std::reference_wrapper<Data1>> &gs, Data1 &p) {
        p.breadcrumb1 = "data1 - ";
        for (auto g: gs) {
    ...
    using CompositeTrait = composite::TraitCompose<Trait1, Trait2, Trait3>::Trait;
}

TEST_CASE("test_composite3 | TraitGroup create", "[test_composite]") {
    auto group = std::make_shared<CompositeTrait::TraitGroup>("g1-1", "g1-2", "g1-3");
    REQUIRE(group->data1 == "g1-1");
    REQUIRE(group->data2 == "g1-2");
    REQUIRE(group->data3 == "g1-3");
}

TEST_CASE("test_composite3 | TraitPart create", "[test_composite]") {
    auto part = std::make_shared<CompositeTrait::TraitPart>("p1-1", "p1-2", "p1-3");
...
TEST_CASE("test_composite3 | Add part", "[test_composite]") {
    auto group = std::make_shared<CompositeTrait::TraitGroup>("g1-1", "g1-2", "g1-3");
    auto part = std::make_shared<CompositeTrait::TraitPart>("p1-1", "p1-2", "p1-3");
    group->addPart(part);
...
TEST_CASE("test_composite3 | Visitor composite", "[test_composite]") {
    auto group1 = std::make_shared<CompositeTrait::TraitGroup>("g1-1", "g1-2", "g1-3");

    auto part1 = std::make_shared<CompositeTrait::TraitPart>("p1-1", "p1-2", "p1-3");
    group1->addPart(part1);

    auto group2 = std::make_shared<CompositeTrait::TraitGroup>("g2-1", "g2-2", "g2-3");
    auto part2 = std::make_shared<CompositeTrait::TraitPart>("p2-1", "p2-2", "p2-3");
